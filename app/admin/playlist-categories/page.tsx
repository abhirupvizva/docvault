...
export default function PlaylistCategoriesPage() {
  ...
  const fetchCategories = async () => {
    try {
      ...
      console.error(error)
      toast.error('Failed to load playlist categories')
    } finally {
      setLoading(false)
    }
  }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const url = editingCategory
        ? `/api/playlist-categories/${editingCategory._id}`
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      const res = await fetch(`/api/playlist-categories/${id}`, {
        method: 'DELETE',
      })
  ...
  const handleEdit = (category: PlaylistCategory) => {
    ...
    <div className="min-h-screen bg-zinc-950 text-white">
      <header className="sticky top-0 z-50 border-b border-zinc-800 bg-zinc-950/80 backdrop-blur-lg">
        <div className="max-w-7xl mx-auto px-4 py-4 flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div className="w-px h-6 bg-zinc-800" />
            <h1 className="text-lg font-semibold text-emerald-500">Playlist Categories</h1>
          </div>
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        <div className="flex items-center justify-between mb-8">
          <div>
            <h2 className="text-3xl font-bold">Manage Playlist Categories</h2>
            <p className="text-zinc-400 mt-2">Create and organize playlist categories</p>
          </div>
          <Dialog
            open={open}
            onOpenChange={(val) => {
            ...
            <DialogContent className="bg-zinc-900 border-zinc-800 text-white">
              <DialogHeader>
                ...
                <DialogDescription className="text-zinc-400">
                  {editingCategory ? 'Update category details below.' : 'Create a new category for playlists.'}
                </DialogDescription>
              </DialogHeader>
